...
public:
	...
	virtual int pType(bc::Baritem* item) = 0;
...
{
	std::function<Barscalar(std::ifstream& stream)> parseBarscal;
...
public:
	...
	BinStateReader() : stream(mainStream)
	{ }
	...
	int pType(bc::Baritem*) override
	{
		...
		case BarType::BYTE8_1:
			parseBarscal = [](std::ifstream &stream) {
				Barscalar scal;
...
{
	std::function<void(std::ofstream& stream, const Barscalar &scal)> valueFunction;
	std::vector<size_t> memoffs;
...
public:
	...
	int pType(bc::Baritem* item)
	{
		return pType(item->barlines[0]->matr[0].value.type);
	}
	...
	int pType(BarType bt)
	{
		...
		{
		case BarType::BYTE8_1:
			valueFunction = [](std::ofstream& stream, const Barscalar &scal) { stream.write((const char *) &scal.data.b1, 1); };
			break;
		case BarType::BYTE8_3:
			valueFunction = [](std::ofstream& stream, const Barscalar &scal) { stream.write((const char *) scal.data.b3, 3); };
			break;
		case BarType::FLOAT32_1:
			valueFunction = [](std::ofstream& stream, const Barscalar &scal) {stream.write((char*)&scal.data.f, 4); };
			break;
		default:
			break;
		}
