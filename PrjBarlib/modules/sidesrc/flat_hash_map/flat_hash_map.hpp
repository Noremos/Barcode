...
{
    template <class T>
    T mmmmin(const T& a, const T& b)
    {
        return a < b ? a : b;
    }

    template <class T>
    T mmmmax(const T& a, const T& b)
    {
        return a > b ? a : b;
    }
...
public:
    ...
    {
        num_buckets = mmmmax(num_buckets, static_cast<size_t>(std::ceil(num_elements / static_cast<double>(_max_load_factor))));
        if (num_buckets == 0)
        {
            reset_to_empty_state();
            return;
    ...
    iterator erase(const_iterator begin_it, const_iterator end_it)
    {
        ...
        if (end_it == this->end())
            return this->end();
        ptrdiff_t num_to_move = mmmmin(static_cast<ptrdiff_t>(end_it.current->distance_from_desired), end_it.current - begin_it.current);
        ...
        for (EntryPointer it = end_it.current; !it->is_at_desired_position();)
        {
            ...
            it->destroy_value();
            ++it;
            num_to_move = mmmmin(static_cast<ptrdiff_t>(it->distance_from_desired), num_to_move);
        }
        return { to_return };
    }
...
private:
    ...
    static int8_t compute_max_lookups(size_t num_buckets)
    {
        ...
        return mmmmax(detailv3::min_lookups, desired);
    }

    size_t num_buckets_for_reserve(size_t num_elements) const
    {
        return static_cast<size_t>(std::ceil(num_elements / mmmmin(0.5, static_cast<double>(_max_load_factor))));
    }
    void rehash_for_other_container(const sherwood_v3_table & other)
    {
        rehash(mmmmin(num_buckets_for_reserve(other.size()), other.bucket_count()));
    }
    ...
    void grow()
    {
        rehash(mmmmax(size_t(4), 2 * bucket_count()));
    }
...
struct fibonacci_hash_policy
{
    ...
    int8_t next_size_over(size_t & size) const
    {
        size = mmmmax(size_t(2), detailv3::next_power_of_two(size));
