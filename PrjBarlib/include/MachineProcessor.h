...
public:
	// Факторы поведения
	float totalMtrixCountMin; // Кол-во в процентах
	float totalMtrixCountMax; // Кол-во в процентах
	int minDepth;
	int maxDepth;
...
public:
	...
	static void fullProcess(const InitConteiter& cont)
	{
		...
		for (int i = 0; i < bestSize; i++)
		{
			...
			//sad.maxStart = Barscalar((uchar)'я', (uchar)'Ь', (uchar)'я');
			//sad.minLen = Barscalar(150, BarType::BYTE8_3);
			//sad.maxLen = Barscalar('\x15', 'ч', 255);
			//sad.initRandomColor();
			std::cout <<
				"Min d:" << as.minDepth <<
				"Max d:" << as.maxDepth <<
